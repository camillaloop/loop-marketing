 ...
 * GET /api/cohort-vc
 *
 * Returns cohort attribution data for Impact Loop VC.
...
const VC_LIST_ID   = "b46477bf08";
const COHORT_START = "2026-03-02"; // week 10 — earliest cohort

// Weekly channel costs (SEK)
const CHANNEL_COSTS = { apollo: 1259, linkedin: 2500, organic: 0 };

// LTV constants for VC
const AVG_PRICE_YEAR  = 5400;  // SEK/year (approximate, based on Stripe data)
...
module.exports = async function handler(req, res) {
  ...
  const [stripeMap, members] = await Promise.all([
    ...
    getAllMailchimp(
      `${mcBase}/lists/${VC_LIST_ID}/members?since_timestamp_opt=${COHORT_START}T00:00:00+00:00&status=subscribed&fields=members.email_address,members.timestamp_opt,members.tags,total_items`,
      mcAuth,
      d => d.members || []
    ),
  ]);
  ...
  for (const { week, monday, channel, members: gm } of Object.values(groups)) {
    ...
    // Initialize windows based on measurability
  ...
  // Sort: newest week first, then channel
